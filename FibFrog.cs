...
class Solution {
    public int solution(int[] A) {
        ...
        for (int i = START; i < GOAL; i++) {
            // If the current position is the Start position or a leaf
            if (i == START || greedy[i] > 0) {
                // Mark all reachable leaves from the current leaf (or start position)
                for (int j = 0; (i + fibs[j]) < GOAL; j++) {
                    // Frog jumps to "i + fibs[j]"
                    int jumpToIndex = i + fibs[j];

                    // Reached the goal (index of GOAL is GOAL - 1)
                    // or reached a leaf
                    if (jumpToIndex == GOAL - 1 || A[jumpToIndex] == 1) {
                        // If the current position is the start, then the frog can reach the jumpToIndex leaf by 1 time jump
